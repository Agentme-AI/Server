...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as AgentMeApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as AgentMeApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  ...
  if (host.tab === "agents") {
    ...
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as AgentMeApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as AgentMeApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as AgentMeApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as AgentMeApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as AgentMeApp);
    await loadDevices(host as unknown as AgentMeApp);
    await loadConfig(host as unknown as AgentMeApp);
    await loadExecApprovals(host as unknown as AgentMeApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as AgentMeApp);
    await loadConfig(host as unknown as AgentMeApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as AgentMeApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as AgentMeApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__AGENTME_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AgentMeApp, false),
    loadPresence(host as unknown as AgentMeApp),
    loadSessions(host as unknown as AgentMeApp),
    loadCronStatus(host as unknown as AgentMeApp),
    loadDebug(host as unknown as AgentMeApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AgentMeApp, true),
    loadConfigSchema(host as unknown as AgentMeApp),
    loadConfig(host as unknown as AgentMeApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AgentMeApp, false),
    loadCronStatus(host as unknown as AgentMeApp),
    loadCronJobs(host as unknown as AgentMeApp),
  ]);
}
